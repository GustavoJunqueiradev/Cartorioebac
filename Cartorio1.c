...
int registro() //Função responsável por cadastrar o usuário no sistema
{
	...
	file = fopen(arquivo, "w"); //cria o arquivo no BD e "w" é write
	...
	file = fopen(arquivo,"a");
	fprintf(file,","); //adicionado vírgula para melhor entendimento quando for consultar
	fclose(file);
	...
	system ("pause"); //pausa no sistema para verificação
...
int main ()
{
	...
	for(laco=1;laco=1;)
	{
		...
		scanf ("%d", &opcao); //Armazenando a escolhe do usuário
